...
class RTPMIDI : public MIDIReadWrite,
                private mbed::NonCopyable<RTPMIDI> {
public:
    RTPMIDI() = delete;
    ...
    bool write(MIDIMessage m) override;
    bool readable() override;
    bool read(MIDIMessage *m) override;
    void attach(mbed::Callback<void()> callback) override;

    void connect(uint16_t control_port);
private:
    uint32_t _ssrc;
    std::string _name;
